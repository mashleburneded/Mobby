...
class UniversalIntentExecutor:
    ...
    async def _get_crypto_price(self, symbol: str, vs_currency: str = 'usd') -> Dict[str, Any]:
        """Get real-time crypto price from public APIs"""
        try:
            from public_api_endpoints import get_crypto_price_public
            result = await get_crypto_price_public(symbol, vs_currency)
    ...
    async def _get_yield_opportunities(self, min_apy: float = 5.0, risk_level: str = 'medium') -> Dict[str, Any]:
        ...
        try:
            from public_api_endpoints import get_defi_data_public
            result = await get_defi_data_public()
    ...
    async def _calculate_portfolio_metrics(self, holdings: Dict[str, float], timeframe: str = '30d') -> Dict[str, Any]:
        ...
        try:
            ...
            for symbol, amount in holdings.items():
                try:
                    ...
                    if 'error' not in price_data and price_data.get('price', 0) > 0:
                        current_value = price_data['price'] * amount
            ...
            if total_value <= 0 or not portfolio_data:
                return {
                    'error': 'Could not fetch price data for any holdings',
                    'total_value': 0,
                    'holdings': {},
                    'metrics': {
                        'total_assets': 0,
            ...
            for symbol in portfolio_data:
                portfolio_data[symbol]['allocation'] = (portfolio_data[symbol]['value'] / total_value) * 100

            # Calculate portfolio-level metrics
            weighted_change_24h = sum(
                (data['change_24h'] * data['allocation'] / 100)
            ...
            return {
                ...
                'metrics': {
                    ...
                    'weighted_change_24h': weighted_change_24h,
                    'largest_holding': max(portfolio_data.items(), key=lambda x: x[1]['allocation'])[0] if portfolio_data else None,
