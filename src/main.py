    ...
    try:
        ...
        try:
            ...
            if should_convert and nl_metadata['confidence'] >= 0.6:
                logger.info(f"Natural language converted: '{processed_text}' -> {command_string} "
                ...
                if len(parts) > 0:
                    ...
                    # Create a fake update for the command
                    fake_text = command_string
                    update.effective_message.text = fake_text
                    context.args = args
            ...
            # If natural language processing didn't work, try enhanced analysis
...
def main():
    ...
    try:
        ...
        application = Application.builder().token(config.get('TELEGRAM_BOT_TOKEN')).post_init(post_init).job_queue(JobQueue()).build()
        ...
        try:
            ...
            try:
                import asyncio
                asyncio.run(stop_mcp_integration())
            except Exception as e:
                logger.error(f"Error stopping MCP integration: {e}")
        ...
        # Ensure MCP servers are stopped even on error
        try:
            import asyncio
            asyncio.run(stop_mcp_integration())
        except:
