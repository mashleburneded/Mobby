...
class DeFiLlamaAPI:
    ...
    async def _make_request(self, endpoint: str, params: Dict = None) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Timeout error for DeFiLlama request: {url}")
            return None
        except Exception as e:
            logger.error(f"Error making DeFiLlama request to {url}: {e}")
            return None
    ...
    async def get_protocol_summary(self, protocol_slug: str) -> Optional[str]:
        ...
        try:
            tvl_formatted = f"${float(tvl_value):,.2f}" if tvl_value and str(tvl_value).replace('.', '').replace('-', '').isdigit() else f"${tvl_value}"
        except (ValueError, TypeError):
            tvl_formatted = f"${tvl_value}"
            
        try:
            change_1d_formatted = f"{float(change_1d_value):+.2f}%" if change_1d_value and str(change_1d_value).replace('.', '').replace('-', '').isdigit() else f"{change_1d_value}%"
        except (ValueError, TypeError):
            change_1d_formatted = f"{change_1d_value}%"
            
        try:
            change_7d_formatted = f"{float(change_7d_value):+.2f}%" if change_7d_value and str(change_7d_value).replace('.', '').replace('-', '').isdigit() else f"{change_7d_value}%"
        except (ValueError, TypeError):
            change_7d_formatted = f"{change_7d_value}%"
        
        summary += f"💰 **TVL:** {tvl_formatted}\n"
        summary += f"📈 **24h Change:** {change_1d_formatted}\n"
        summary += f"📊 **7d Change:** {change_7d_formatted}\n"
