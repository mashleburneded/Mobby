...
class EnhancedIntentSystem:
    ...
    def _initialize_intent_patterns(self) -> Dict[IntentType, List[str]]:
        ...
        return {
            ...
            IntentType.CRYPTO_PRICE: [
                # Direct patterns like "BTC price", "ETH value"
                r"^(btc|bitcoin|eth|ethereum|sol|solana|ada|cardano|dot|polkadot|matic|polygon|avax|avalanche|link|chainlink|uni|uniswap|aave|comp|compound|mkr|maker|crv|curve|yfi|yearn|sushi|sushiswap|1inch|snx|synthetix|bal|balancer)\s+(price|cost|value|worth|rate)$",
                r"^(price|cost|value|worth)\s+(btc|bitcoin|eth|ethereum|sol|solana|ada|cardano|dot|polkadot|matic|polygon|avax|avalanche|link|chainlink|uni|uniswap|aave|comp|compound|mkr|maker|crv|curve|yfi|yearn|sushi|sushiswap|1inch|snx|synthetix|bal|balancer)$",
                # Just token name (assume price query) - exclude DeFi protocol names
                r"^(btc|bitcoin|eth|ethereum|sol|solana|ada|cardano|dot|polkadot|matic|polygon|avax|avalanche|link|chainlink)$",
            ...
            IntentType.PORTFOLIO_CHECK: [
                ...
                r"^(portfolio|holdings|balance|assets|wallet)$",
                r"^(my\s+)?(portfolio|holdings|balance|assets|wallet)$",
                r"^show\s+(my\s+)?(portfolio|holdings|balance|assets|wallet)$",
                r"^check\s+(my\s+)?(portfolio|holdings|balance|assets|wallet)$",
                # Traditional patterns
                r"(?:my\s+)?portfolio",
                ...
                r"account\s+balance",
                r"what'?s\s+in\s+my\s+(?:portfolio|wallet)",
                r"how\s+much\s+do\s+i\s+have",
            ],
            
            IntentType.ALERT_MANAGEMENT: [
                # Direct alert queries
                r"^(alert|alerts|notify|notification|notifications)$",
                r"^(set|create|add|make)\s+(alert|alerts)$",
                r"^(show|list|check)\s+(alert|alerts)$",
                r"^(delete|remove|cancel)\s+(alert|alerts)$",
                # Traditional patterns
                ...
                r"monitor\s+(\w+)",
                r"track\s+(\w+)",
                r"alert\s+me\s+(?:about|for)",
                r"set\s+up\s+(?:an?\s+)?alert",
                r"i\s+want\s+(?:an?\s+)?alert",
            ],
            
            IntentType.HELP_REQUEST: [
                # Direct help queries
                r"^(help|assistance|support|commands|guide|tutorial|instructions)$",
                r"^(what\s+can\s+you\s+do|what\s+do\s+you\s+do|how\s+do\s+you\s+work)$",
                r"^(show\s+)?(commands|features|capabilities|help)$",
                ...
                r"i\s+need\s+help",
                r"can\s+you\s+help",
                r"what\s+are\s+your\s+(?:commands|features)",
            ...
            IntentType.DEFI_PROTOCOL: [
                ...
                r"^(uniswap|aave|compound|maker|curve|yearn|sushi|1inch|balancer|synthetix)$",
                r"^(what\s+is|tell\s+me\s+about|info\s+about)\s+(uniswap|aave|compound|maker|curve|yearn|sushi|1inch|balancer|synthetix)$",
                r"^(uniswap|aave|compound|maker|curve|yearn|sushi|1inch|balancer|synthetix)\s+(info|data|stats|tvl|protocol)$",
                # Traditional patterns
            ...
            IntentType.GREETING: [
                ...
                r"^(?:good\s+to\s+see\s+you|glad\s+to\s+see\s+you)$",
            ],
            
            IntentType.EXPLANATION: [
                # High priority explanation patterns for general concepts
                r"^(?:what\s+is|explain|define)\s+(defi|blockchain|crypto|cryptocurrency|bitcoin|ethereum|staking|yield\s+farming|smart\s+contracts|dapps|nfts)$",
                r"^how\s+(?:does|do)\s+(defi|blockchain|crypto|cryptocurrency|staking|yield\s+farming|smart\s+contracts|dapps|nfts)\s+work$",
                # Traditional patterns
                ...
                r"(?:what\s+are|explain)\s+(?:smart\s+contracts|dapps|nfts|defi\s+protocols)",
                r"(?:can\s+you\s+)?explain\s+(defi|blockchain|crypto|staking|yield\s+farming)",
            ],
            
            IntentType.GENERAL_QUERY: [
                # Catch-all for conversational queries
                r"(?:can\s+you|are\s+you\s+able\s+to|do\s+you)",
                r"(?:i\s+want\s+to|i\s+need\s+to|i\s+would\s+like\s+to)",
                r"(?:tell\s+me|show\s+me|give\s+me)",
                r"(?:what\s+about|how\s+about)",
                r"(?:is\s+it\s+possible|can\s+i)",
                r"(?:i\s+have\s+a\s+question|question)",
            ],
        }
    ...
    async def analyze_intent(self, text: str, user_id: int, context: Dict = None) -> EnhancedIntentAnalysis:
        ...
        # Check for explanation queries (high priority for educational content)
        analysis = self._check_intent_patterns(text_lower, IntentType.EXPLANATION)
    ...
    def _extract_entities(self, match: re.Match, intent_type: IntentType) -> Dict[str, Any]:
        ...
        if intent_type == IntentType.CRYPTO_PRICE:
            if match.groups():
                ...
                # Handle patterns without groups (like single token names)
                matched_text = match.group(0).lower()
                # Extract token from the matched text
                tokens = ['btc', 'bitcoin', 'eth', 'ethereum', 'sol', 'solana', 'ada', 'cardano', 
                         'dot', 'polkadot', 'matic', 'polygon', 'avax', 'avalanche', 'link', 
                         'chainlink', 'uni', 'uniswap', 'aave', 'comp', 'compound', 'mkr', 
                         'maker', 'crv', 'curve', 'yfi', 'yearn', 'sushi', 'sushiswap', 
                         '1inch', 'snx', 'synthetix', 'bal', 'balancer']
                for token in tokens:
                    if token in matched_text:
                        entities['symbol'] = token
                        entities['normalized_symbol'] = self.crypto_symbols.get(token, token)
                        break
